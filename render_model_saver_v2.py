...
def main(args):
    ...
    dim = args.dim
    # dim 最好不要超过64
    # pts_emb = tf.placeholder(tf.float32, shape=[None, dim, dim, 63])
    pts_emb = tf.placeholder(tf.float32, shape=[None, dim, dim, 64])
    # dir_emb = tf.placeholder(tf.float32, shape=[None, dim, dim, 27])
    dir_emb = tf.placeholder(tf.float32, shape=[None, dim, dim, 64])
    # first linear [63, 256]
    w1 = weight_variable([64, 256])
    ...
    view_w = weight_variable([256+64, 128])
    ...
    rgb_out = tf.add(tf.matmul(view_out, rgb_w), rgb_b, name='rgb')
