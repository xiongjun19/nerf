...
def save_alpha_model(model_path, dim, pt_dim):
    ...
    hid_dim = 256
    # first linear [63, 256]
    w1 = weight_variable([pt_dim, hid_dim])
    ...
    for i in range(layer_nums):
        _shp = [hid_dim, hid_dim]
        if i == concat_idx:
            # _shp = [319, 256]
            _shp = [pt_dim + hid_dim, hid_dim]
    ...
    # get alpha_out
    alpha_w = weight_variable([hid_dim, 64])
...
def save_rgb_model(model_path, dim, view_dim):
    ...
    feat_w = weight_variable([hid_dim, hid_dim])
    feat_b = weight_variable([hid_dim])
    feat_out = tf.matmul(h, feat_w) + feat_b # [b, dim, hid_dim]
    input_viewdirs = tf.concat([feat_out, dir_emb], -1) # [b, dim, hid_dim + 28]
    view_w = weight_variable([hid_dim+view_dim, 128])
